"""
This module defines the MemProfiler class and the WSGI Middleware (
ProfilerMiddleware).
"""
...
class MemProfiler:
    """
    A singleton class that records and logs memory usage using the Pympler 
    library
    """
    ...
    class __MemProfiler:
        ...
        def log_diff(self, msg=None):
            ...
            function was called. It logs the result to a file and returns it 
            as a string. The string in the 'msg' parameter is included at the 
            top of the results.
            """
        ...
        def log_diff_req(self, req):
            ...
            if self.on:
                msg='Request: %s %s\nBody: %s' % \
                    (req.method, req.path_qs, req.body if req.body else\
                            '(EMPTY)')
                self.log_diff(msg)

        def log_diff_res(self, req, res):
            """
            Same as log_diff, but receives a pyramid request 'req'
            and a pyramid response 'res' and prints them along with
            the results to identify which request was called.
            """
        ...
        def print_diff(self):
            """
            Same as log_diff, but prints to console instead of a
            log file
            """
    ...
    def __init__(self):
        """
        Returns the singleton instance of the MemProfiler class. It
        creates the object only on the first time it's called.
        """
...
class ProfilerMiddleware:
    ...
    def __call__(self, environ, start_response):
        if environ.get("PATH_INFO") == "/__memprofiler_dump":
            ...
            if mem_profiler.on:
                ...
                content="Memory Profiler P is OFF.\n\nTo turn it on: " +\
                        "GET /__memprofiler_on\nTo turn it off: " +\
                        "GET /__memprofiler_off"
        ...
        if environ.get("PATH_INFO") == "/__memprofiler_web":
            ...
            start_response("200 OK",[('Content-type' , 'text/html')])
            return [b"Memory Profiler's Web Interface is now running on " +\
                    "port: 8080"]
...
# NOTE: This function is called by Pyramid before and after every request!
# By John Doe
