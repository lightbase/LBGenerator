...
class MemProfiler:
    """
    A singleton class that records and logs memory
    usage using the Pympler library
    """
    class __MemProfiler:
        ...
        def log_diff(self, msg=None):
            """
            Calculates memory usage diff between now and the last
            time this function was called. It logs the result to a
            file and returns it as a string. The string in the 'msg'
            parameter is included at the top of the results.
            """
            if self.on:
        ...
        def log_diff_req(self, req):
            ...
            and prints it along with the results to identify which
            request was called.
            """
            if self.on:
                msg = 'Request: %s %s\nBody: %s' % \
