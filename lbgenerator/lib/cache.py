...
def clear_cache(name=None):
    ...
    for _cache in cache_managers.values():
        if name is not None and _cache == name:
            ...
            # NOTE: Clear only this cache! By John Doe
            _cache.clear()
            break
...
def clear_collection_cache(base):
    ...
    # NOTE: I have to copy the object to avoid "RunTime Error"! By John Doe
    ...
    for manager_key in managers.keys():
        ...
        for cache_key in key_list:
            ...
            # NOTE: Search all keys for this base and remove it! By John Doe
...
def clear_document_cache(base, id_doc):
    ...
    for manager_key in managers.keys():
        ...
        # TODO: Get close_sess from the class. It defaults to True and there's
        # no other possibility! By John Doe
        close_sess="True"
        key="/cached/" + base + "/doc/" + str(id_doc) + " " + str(id_doc) +\
                " " + close_sess
        key=key.encode()
        try:
