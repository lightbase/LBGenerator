...
class BaseContextFactory(CustomContextFactory):
    ...
    def delete_member(self, id):
        ...
        doc_table=get_doc_table(member.name, 
            config.METADATA, 
            **relational_fields
        )
    ...
    def update_column(self, 
        ...
        try:
            ...
            # NOTE: Current_column is the Group/Field object before update!
            # By John Doe
            ...
            old_name=None
            if isinstance(current_column, Field) and\
                    isinstance(new_column, Field):
                if new_column.name != current_column.name:
                    old_name=current_column.name
                    current_column.name=new_column.name
                    name_changed=True
            elif isinstance(current_column, Group) and\
                    isinstance(new_column, Group):
            ...
            if name_changed:
                ...
                json_path_list='[{"path":"%s","fn":null,"mode":"delete",' +\
                        '"args":[]}]' % "/".join(column_path)
                params={
                    'path': json_path_list,
                    'alter_files': False
                }
                ...
                try:
                    ...
                    # NOTE: Current is a dictionary with current column struct!
                    # By John Doe
                    if current is None:
                        # NOTE: Undo delete column! By John Doe
                        self._undo_delete_column(
                            base, 
                            old_name, 
                            column_path, 
                            results
                    ...
                    if current.get('field', None):
                        ...
                        # NOTE: Check if field is relational and change its
                        # name in postgres db! By John Doe
                    ...
                    # NOTE: Reraise exception to return it as error view!
                    # By John Doe
                    raise e
                ...
                try:
                    ...
                    try:
                        ...
                        # NOTE: If current_column is group it doesnt have to
                        # undo alter column! By John Doe
                    ...
                    # NOTE: Reraise exception to be displayed as error msg!
                    # By John Doe
                    raise e
